...
1. Within-run similarity: Does a single prompt run produce duplicate/similar questions?
   - Analyzed from original other_analysis files (60 topics, 4 runs each)
2. Cross-run consistency: How similar are questions when running the same prompt multiple times?
   - Analyzed from Consistency runs directories (5 topics per discipline, 4 runs of 10 questions each)
...
class PromptConsistencyAnalyzer:
    ...
    def __init__(self, base_path):
        ...
        self.within_run_data = None  # From original other_analysis files
        self.cross_run_data = None   # From Consistency runs directories
        
    def load_all_files(self):
        """Load and parse both within-run and cross-run consistency files."""
    ...
    def _load_cross_run_files(self):
        ...
        }within_run_file(self, filepath, discipline, source_folder):
        ...
        for folder, discipline in disciplines.items():
            ...
            if not consistency_path.exists():
                print(f"Warning: Consistency runs folder not found: {consistency_path}")
                continue
                
            files = list(consistency_path.glob('other_analysis_*.csv'))
            print(f"Found {len(files)} consistency files in {folder}")
        ...
        df = pd.DataFrame(all_data)
        print(f"\nTotal consistency topics loaded: {len(df)}")
        print(f"Disciplines: {df['discipline'].unique()}")
        return dffiles:
                try:
    ...
    def _parse_file(self, filepath, discipline, source_folder):
        ...
        try:
            ...
            if not within_run_data:
                print(f"  Warning: No within-run data in {filepath.name}")
                return None
            
            # Calculate within-run metrics only
    ...
    def _parse_cross_run_file(self, filepath, discipline, source_folder):
        ...
        try:
            ...
            if not cross_run_data:
                print(f"  Warning: No cross-run data in {filepath.name}")
                return None
    ...
    def export_data_tables(self, output_dir=None):
        ...
        # 1. Overall summary statistics for within-run
        overall_stats = {
        ...
        discipline_stats = []
        for discipline in self.within_run_data['discipline'].unique():
        ...
        for discipline in self.cross_run_data['discipline'].unique():
